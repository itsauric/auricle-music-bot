...
export class ConnectCommand extends Command {
	...
	public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		...
		if (!(interaction.member instanceof GuildMember))
			return interaction.reply({ embeds: [makeEmbed(`${emojis.error} | This command must be used in a server`)], flags: MessageFlags.Ephemeral });
		...
		if (useQueue(interaction.guild!.id))
			return interaction.reply({ embeds: [makeEmbed(`${emojis.error} | I am **already** in a voice channel`)], flags: MessageFlags.Ephemeral });
		...
		try {
			...
			return interaction.reply({ embeds: [makeEmbed(`${emojis.connect} | **Connected** to the voice channel`)] });
		} catch (error: unknown) {
			this.container.logger.error(error);
			return interaction.reply({ embeds: [makeEmbed(`${emojis.error} | Failed to connect to the voice channel`)], flags: MessageFlags.Ephemeral });
		}
	}
}
